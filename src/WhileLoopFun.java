...
*/
	public int countLetter(String word, String letter) {
		...
		boolean stop = false;
		while (!stop) {
			currIdx = word.indexOf(letter, currIdx) + 1;
			if (currIdx == 0) // "-1"
				stop = true;
			count++;
		}
...
*/
	public int maxDoubles(int number, int threshold) {
		...
		return Math.max(0, count); // in case number > threshold
	}
...
Example:  If number is 1, return false (it has only a single divisor: 1*)
	 ...
	 * 1 is considered non-prime because prime numbers have exactly two divisors - the number and itself -
	 but 1 has only a single divisor! (dont believe it? Google it!)
	 */
	public boolean isPrime(int number) {
		if (number == 1) {
			return false; // thanks mathematicians!
		}
